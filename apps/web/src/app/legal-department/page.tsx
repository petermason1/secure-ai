...
export default function LegalDepartmentPage() {
  ...
  const loadAgents = async () => {
    try {
      const res = await fetch('/api/legal-department/list');
  ...
  const createDepartment = async () => {
    setCreating(true);
    try {
      const res = await fetch('/api/legal-department/create', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        await loadAgents();
        alert(`✅ Created ${data.agents.length} Legal bots!`);
      } else {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {agents.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-8 text-center">
            <p className="text-lg text-slate-300 mb-4">Legal Department not created yet</p>
            <button
            ...
            >
              {creating ? 'Creating...' : 'Create Legal Department'}
            </button>
          </div>
        ) : (
            ...
            {agents.map((agent) => (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <span className="text-2xl">{agent.metadata?.icon || '⚖️'}</span>
                  <div>
