...
export default function HRDepartmentPage() {
  ...
  const createDepartment = async () => {
    setCreating(true);
    try {
      const res = await fetch('/api/hr-department/create', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        await loadAgents();
        alert(`✅ Created ${data.agents.length} HR bots!`);
      } else {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        {agents.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-8 text-center">
            <p className="text-lg text-slate-300 mb-4">HR Department not created yet</p>
            <button
            ...
            >
              {creating ? 'Creating...' : 'Create HR Department'}
            </button>
          </div>
        ) : (
